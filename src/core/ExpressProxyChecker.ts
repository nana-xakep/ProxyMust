...
async function createTestTab(url: string): Promise<number> {
    ...
    console.log(`[ExpressChecker] Created test tab ID=${tab.id}`);
    return tab.id;
}
...
async function removeFromHistory(url: string): Promise<void> {
    ...
    try {
        ...
        console.log(`[ExpressChecker] Removed from history: ${url}`);
    } catch (e) {
        console.warn(`[ExpressChecker] Failed to remove from history: ${url}`, e);
    }
}
...
async function withDirectProxy<T>(fn: () => Promise<T>): Promise<T> {
    ...
    try {
        ...
        console.log(`[ExpressChecker] Временно переключён прямой прокси для получения IP`);
        return await fn();
    } finally {
        await new Promise<void>(resolve => {
            proxyAPI.settings.set({ value: originalConfig || { mode: "system" }, scope: "regular" }, () => resolve());
        });
        console.log(`[ExpressChecker] Прокси восстановлен`);
    }
}

/**
 * English: Get sorted list of IP services by response time.
 * Russian: Получить отсортированный по скорости список IP-сервисов.
 */
    ...
    for (const url of IP_SERVICES) {
        ...
        try {
            ...
            if (response.ok && !response.url.startsWith('https://')) {
                ...
                console.log(`[ExpressChecker] IP-сервис ${url} ответил за ${elapsed}мс (HTTP)`);
            } else if (response.url.startsWith('https://')) {
                console.log(`[ExpressChecker] IP-сервис ${url} редиректит на HTTPS, пропускаем`);
            }
        } catch (e) {
            console.log(`[ExpressChecker] IP-сервис ${url} недоступен или редиректит`);
        }
    }
...
async function findFastestIpService(): Promise<string | null> {
    ...
    pendingFindPromise = (async () => {
        ...
        if (ipServicesRanked && ipServicesRanked.length) {
            ...
            console.log(`[ExpressChecker] Выбран быстрейший IP-сервис: ${fastestIpServiceUrl}`);
...
async function getDirectIp(): Promise<string | null> {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (isValidIPv4(text)) {
                directIp = text;
                console.log(`%c[ExpressChecker] Прямой IP: ${directIp}`, 'color: #0088ff; font-weight: bold; font-size: 1.2em');
                return directIp;
            } else {
                console.warn(`[ExpressChecker] Сервис вернул невалидный IP: ${text}`);
            }
        }
    } catch (e) {
        console.error(`[ExpressChecker] Ошибка получения прямого IP:`, e);
    }
    return null;
}

export async function getCachedDirectIp(): Promise<string | null> {
    return directIp;
}

export async function resetIpCache(): Promise<void> {
    ...
    console.log(`[ExpressChecker] Кэш IP-сервисов и прямого IP сброшен`);
}
...
export async function forceCloseHiddenWindow(): Promise<void> {
    console.log(`[ExpressChecker] forceCloseHiddenWindow called but no hidden window exists`);
}

// ========== Вспомогательная функция получения IP через прокси с защитой от залипания ==========
async function fetchProxyIpViaTab(
    ...
    if (!workingServices.length) {
        console.log(`%c[ExpressChecker] ❌ Нет доступных IP-сервисов`, 'color: #ff0000');
        return null;
    }
    
    const servicesToTry = workingServices.slice(0, 3);
    console.log(`[ExpressChecker] 🔍 Параллельная проверка IP через ${servicesToTry.length} сервисов для прокси ${proxy.host}:${proxy.port}`);
    ...
    const testService = async (serviceUrl: string): Promise<string | null> => {
        ...
        try {
            ...
            console.log(`%c[ExpressChecker] IP-вкладка открыта id=${ipTabId} для ${freshUrl}`, 'color: #ff8800');
            ...
            if (retryForSticky && lastIpForService && lastProxyForService && lastProxyForService !== proxyKey) {
                if (trimmedIp === lastIpForService && trimmedIp !== directIpStr) {
                    console.log(`[ExpressChecker] ⚠️ Сервис ${serviceUrl} залип на IP ${trimmedIp} (был для ${lastProxyForService}, сейчас для ${proxyKey})`);
                    return null;
                }
            }
            lastSeenIpByService.set(serviceUrl, trimmedIp);
            lastProxyKeyByService.set(serviceUrl, proxyKey);
            console.log(`%c[ExpressChecker] Получен IP через прокси: ${trimmedIp}`, 'color: #ff8800; font-weight: bold; font-size: 1.2em');
            return trimmedIp;
        } catch (err) {
            console.log(`[ExpressChecker] Ошибка при проверке IP через ${serviceUrl}:`, err);
            return null;
        } finally {
            if (ipTabId) {
                try { 
    ...
    if (result) {
        console.log(`%c[ExpressChecker] ✅ Получен IP через прокси: ${result}`, 'color: #00ff00');
        return result;
    }
    console.log(`%c[ExpressChecker] ❌ Не удалось получить IP ни от одного сервиса`, 'color: #ff0000');
    return null;
}

// ========== Основная экспресс-проверка ==========
/**
 * Быстрая проверка прокси.
 * @param proxy - проверяемый прокси
 * @param testUrl - URL для загрузки (обычно целевой сайт)
 * @param ipServiceUrl - заранее выбранный быстрый IP-сервис (можно null, будет найден)
 * @param directIpString - заранее полученный прямой IP (если null, будет получен)
 * @param retryForDirectIp - внутренний флаг для повторной проверки при прямом IP
 */
export async function quickCheckProxy(
    proxy: ProxyServer,
    testUrl: string,
    ipServiceUrl: string | null = null,
    directIpString: string | null = null,
    ...
    console.log(`[ExpressChecker] ⚡ Начало проверки ${proxy.host}:${proxy.port} для ${testUrl}`);

    let fastestIpService = ipServiceUrl;
    let directIpStr = directIpString;
    if (!fastestIpService || !directIpStr) {
        await withDirectProxy(async () => {
            if (!fastestIpService) fastestIpService = await findFastestIpService();
            if (!directIpStr && fastestIpService) directIpStr = await getDirectIp();
            return;
        });
    }
    if (!fastestIpService) {
        console.warn(`[ExpressChecker] Не удалось определить IP-сервис, IP-сравнение отключено`);
    }
    ...
    const decideFinalResult = async () => {
        ...
        if (ipCheckPromise && !ipCheckCompleted) {
            console.log(`[ExpressChecker] Ожидание завершения IP-проверки перед принятием решения...`);
        ...
        if (hasIndirectSuccess) {
            if (mainTabFailed) {
                console.log(`%c[ExpressChecker] Косвенный успех: основная вкладка не загрузилась, но IP получен не прямой`, 'color: #0088ff');
                await finishTest(true, false);
                return;
            }
            if (!extendedTimer) {
                console.log(`[ExpressChecker] Косвенный успех получен, продлеваем ожидание до 20 секунд...`);
                extendedTimer = setTimeout(() => {
                    if (!testCompleted && hasIndirectSuccess && !exactSuccessTriggered) {
                        console.log(`%c[ExpressChecker] Косвенный успех: IP получен, но целевой сайт не загружен`, 'color: #ffaa00');
        ...
        if (directIpDetected && retryForDirectIp) {
            console.log(`[ExpressChecker] Прямой IP, повторная проверка...`);
    ...
    try {
        ...
        console.log(`[ExpressChecker] Вкладка открыта id=${tabId}`);

        ipCheckPromise = new Promise<string | null>((resolve) => {
            ipCheckPromiseResolve = resolve;
        });
        
        if (fastestIpService && !ipCheckStarted) {
            ipCheckStarted = true;
            (async () => {
                if (testCompleted) return;
                console.log(`[ExpressChecker] Запуск параллельной проверки IP через прокси...`);
                ...
                if (ip) {
                    retrievedIp = ip;
                    console.log(`[ExpressChecker] Параллельная проверка вернула IP: ${ip}`);
                    if (directIpStr && ip === directIpStr) {
                        console.log(`%c[ExpressChecker] ⚠️ Прокси вернул прямой IP (${ip}) – не считаем успехом`, 'color: #3a86ff');
                        directIpDetected = true;
                    } else {
                        console.log(`%c[ExpressChecker] IP-вкладка вернула не прямой IP: ${ip}`, 'color: #ffa500');
                        hasIndirectSuccess = true;
                    }
                    await decideFinalResult();
                } else {
                    console.log(`[ExpressChecker] IP-вкладка не вернула IP`);
                    await decideFinalResult();
                }
            })().catch(e => console.log(`[ExpressChecker] Ошибка в параллельной проверке IP: ${e}`));
        } else {
            ipCheckCompleted = true;
            if (ipCheckPromiseResolve) ipCheckPromiseResolve(null);
        }
            ...
            if (info.title) {
                ...
                if (CERT_ERROR_INDICATORS.some(e => titleLower.includes(e.toLowerCase()))) {
                    if (!exactSuccessTriggered) {
                        ...
                        console.log(`%c[ExpressChecker] ★ Ошибка сертификата (прокси работает)`, 'color: #00ff00');
                        decideFinalResult();
                    }
                    return;
                }
                if (ERROR_INDICATORS.some(e => titleLower.includes(e.toLowerCase()))) {
                    console.log(`[ExpressChecker] ❌ Общая ошибка в заголовке, прокси не работает`);
                    mainTabFailed = true;
                    decideFinalResult();
                    return;
                }
        ...
        if (webReq && webReq.onResponseStarted) {
            webListener = (details: any) => {
                ...
                if (details.statusCode === 200 && !exactSuccessTriggered) {
                    exactSuccessTriggered = true;
                    console.log(`%c[ExpressChecker] ★ Получен ответ 200`, 'color: #00ff00');
                    decideFinalResult();
                }
            };
        ...
        faviconInterval = setInterval(async () => {
            ...
            try {
                ...
                if (favUrl) {
                    ...
                    if (isCertError && !exactSuccessTriggered) {
                        exactSuccessTriggered = true;
                        console.log(`%c[ExpressChecker] ★ Favicon с ошибкой сертификата`, 'color: #00ff00');
                        decideFinalResult();
                        return;
                    }
                    if (!isErrorFav && !exactSuccessTriggered) {
                        exactSuccessTriggered = true;
                        console.log(`%c[ExpressChecker] ★ Favicon загружен`, 'color: #00ff00');
                        decideFinalResult();
                        return;
                    }
                }
        ...
        const totalTimeout = setTimeout(async () => {
            if (!testCompleted && !exactSuccessTriggered) {
                console.log(`%c[ExpressChecker] Первый таймаут (8 сек) – нет ответа 200`, 'color: #ffaa00');
                mainTabFailed = true;
                decideFinalResult();
            }
        }, 8000);
        mainTimeoutId = totalTimeout;
        ...
        if (!finalResult.alive && directIpDetected && retryForDirectIp && !finalResult.exact) {
            console.log(`[ExpressChecker] Прокси вернул прямой IP. Повторяем проверку один раз...`);
            await cleanup(true);
            return quickCheckProxy(proxy, testUrl, fastestIpService, directIpStr, false);
        }
        
        if (finalResult.alive && finalResult.exact) {
            console.log(`%c[ExpressChecker] Результат: ${proxy.host}:${proxy.port} = ЖИВ (${latency}мс)`, 'color: #00ff00; font-weight: bold');
            return { alive: true, latencyMs: latency, ip: retrievedIp || undefined };
        } else if (finalResult.alive && !finalResult.exact) {
            console.log(`%c[ExpressChecker] Результат: ${proxy.host}:${proxy.port} = КОСВЕННЫЙ УСПЕХ (${latency}мс)`, 'color: #0088ff; font-weight: bold');
            return { alive: true, latencyMs: latency, error: "INDIRECT", ip: retrievedIp || undefined };
        } else {
            console.log(`%c[ExpressChecker] Результат: ${proxy.host}:${proxy.port} = МЁРТВ (${latency}мс)`, 'color: #ff3333; font-weight: bold');
            if (directIpDetected && !retryForDirectIp) {
                return { alive: false, latencyMs: latency, error: "DIRECT_IP", ip: retrievedIp || undefined };
            }
            return { alive: false, latencyMs: latency, error: finalResult.error || "Прокси не работает" };
        }

    } catch (err: any) {
        console.error(`[ExpressChecker] Критическая ошибка:`, err);
        await cleanup(true);
...
// Экспорт вспомогательных функций для внешнего использования (если нужно)
export { findFastestIpService, getDirectIp };