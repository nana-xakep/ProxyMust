...
// ==================== Visual Mode (debug) ====================
// English: Set to true to see tabs during testing (debug mode)
// Russian: Установите true для визуального отображения вкладок во время тестирования (режим отладки)
...
async function createTestTab(url: string): Promise<number> {
    ...
    console.log(`[CycleTester] Created test tab ID=${tab.id}`);
    return tab.id;
}
...
async function removeFromHistory(url: string): Promise<void> {
    ...
    try {
        ...
        console.log(`[CycleTester] Removed from history: ${url}`);
    } catch (e) {
        console.warn(`[CycleTester] Failed to remove from history: ${url}`, e);
    }
}

// ==================== IP Service Rotation Mechanism ====================
...
async function withDirectProxy<T>(fn: () => Promise<T>): Promise<T> {
    ...
    try {
        ...
        console.log(`[CycleTester] Временно переключён прямой прокси для получения IP`);
        return await fn();
    } finally {
        await new Promise<void>(resolve => {
            proxyAPI.settings.set({ value: originalConfig || { mode: "system" }, scope: "regular" }, () => resolve());
        });
        console.log(`[CycleTester] Прокси восстановлен`);
    }
}

function getProxyListFromSettings(): ProxyListItem[] {
    console.log("[CycleTester] Reading proxies directly from Settings.current...");
    if (!Settings.current || !Settings.current.proxyServers) {
        console.error("[CycleTester] Settings.current or proxyServers not available");
        return [];
    }
    ...
    console.log(`[CycleTester] Read ${manualProxies.length} manual + ${subscribedProxies.length} subscription = ${allProxies.length} total proxies`);
...
async function getDirectIpBackground(): Promise<string | null> {
    console.log("[CycleTester] Getting direct IP...");
    if (environment.name === "Firefox") {
        console.log("[CycleTester] Using popup method for Firefox");
        ...
        if (result.workingServices && result.workingServices.length) {
            ...
            fastestIpServiceUrl = result.workingServices[0] || null;
        } else {
            ...
            try {
                ...
                if (response.ok && !response.url.startsWith('https://')) {
                    ...
                    if (isValid) {
                        ...
                        console.log(`[CycleTester] IP-сервис ${service} вернул ${ip} за ${Date.now() - start}мс`);
                    }
                }
            } catch (e) {
                console.log(`[CycleTester] IP-сервис ${service} недоступен`);
            }
        }
        if (workingServices.length === 0) {
            console.log("[CycleTester] Не найден доступный IP-сервис");
            return null;
        }
        workingServices.sort((a, b) => a.time - b.time);
        const fastestService = workingServices[0];
        const fastestIp = fastestService.ip;
        console.log(`%c[CycleTester] ПРЯМОЙ IP: ${fastestIp} (через ${fastestService.url} за ${fastestService.time}мс)`, 'color: #00aaff; font-weight: bold; font-size: 1.2em');
        ipServicesRanked = workingServices.map(s => s.url);
        fastestIpServiceUrl = fastestService.url;
        console.log(`[CycleTester] Работающие IP-сервисы (${ipServicesRanked.length}):`, ipServicesRanked);
        console.log(`[CycleTester] fastestIpServiceUrl = ${fastestIpServiceUrl}`);
        return fastestIp;
    });
}

async function getDirectIpViaPopup(): Promise<{ directIp: string | null; workingServices: string[] }> {
    console.log("[CycleTester] Opening popup to get direct IP and working services...");
    return new Promise<{ directIp: string | null; workingServices: string[] }>((resolve) => {
        ...
        const handler = (message: any) => {
            if (message && message.command === "DIRECT_IP_RESULT") {
                ...
                console.log("[CycleTester] Received direct IP from popup:", ip);
                console.log("[CycleTester] Working services from popup:", services);
        ...
        timeoutId = setTimeout(() => {
            api.runtime.onMessage.removeListener(handler);
            console.warn("[CycleTester] Timeout waiting for direct IP from popup");
        ...
        api.tabs.create({ url: popupUrl, active: false }).catch(err => {
            if (timeoutId) clearTimeout(timeoutId);
            api.runtime.onMessage.removeListener(handler);
            console.error("[CycleTester] Failed to open popup for IP detection:", err);
            resolve({ directIp: null, workingServices: [] });
        });
    });
}
...
export const ProxyCycleTester = {
    ...
    _extendedTimer: null as ReturnType<typeof setTimeout> | null,
    _extendedTimerStarted: false,
    _indirectTimeoutFired: false,
    ...
    async startCycleTest(testSite: string, refreshTabSetting?: boolean, originalProfileId?: string | null, proxyList?: ProxyListItem[]): Promise<void> {
        try {
            ...
            if (this._isRunning) {
                console.log("[CycleTester] Test already running");
                return;
            }
            if (!Settings.current || !Settings.current.proxyServers) {
                console.log("[CycleTester] Settings not ready, waiting for initialization...");
                await new Promise<void>((resolve) => {
                    const onInit = () => {
                ...
                console.log("[CycleTester] Settings initialized");
            }
            ...
            if (originalProfileId) {
                console.log(`[CycleTester] Original profile ID: ${originalProfileId}`);
            }
            ...
            if (proxyList && proxyList.length > 0) {
                ...
                console.log(`[CycleTester] Using provided proxy list with ${proxyList.length} proxies`);
            } else {
                this._proxiesList = getProxyListFromSettings();
                if (this._proxiesList.length === 0) {
                    console.error("[CycleTester] No proxies found in settings");
                    console.log("[CycleTester] Please ensure proxies are added in Settings page");
                    return;
                }
            }
            ...
            }).catch((err) => console.warn("[CycleTester] Could not get current window ID:", err));
            ...
            if (!this._windowRemovedListener) {
                this._windowRemovedListener = (windowId: number) => {
                    if (this._isRunning && this._mainWindowId !== null && windowId === this._mainWindowId) {
                        console.log("[CycleTester] Main window closed, cleaning up...");
                        this.cancelTest();
                    }
                };
            ...
            if (!this._cancelMessageListener) {
                this._cancelMessageListener = (message: any) => {
                    ...
                    if (command === "CANCEL_PROXY_TEST_FOR_SITE" || 
                        ...
                        console.log(`[CycleTester] Received cancellation command: ${command}`);
                        this.cancelTest();
                    }
                };
                api.runtime.onMessage.addListener(this._cancelMessageListener);
                console.log(`[CycleTester] Cancellation message listener registered`);
            }

            this._directIp = await this.getDirectIpWithCancellation();
            if (this._cancelRequested) {
                console.log("[CycleTester] Cancelled during direct IP detection, exiting.");
                api.runtime.sendMessage({ command: "TEST_CANCELLED" });
                return;
            }
            console.log(`%c[CycleTester] Прямой IP: ${this._directIp || "не получен"}`, 'color: #00aaff; font-weight: bold');

            if (!ipServicesRanked || ipServicesRanked.length === 0) {
                console.warn(`[CycleTester] Список IP-сервисов пуст, используем полный список как fallback`);
                ipServicesRanked = [...IP_SERVICES];
                fastestIpServiceUrl = ipServicesRanked[0];
            }
    ...
    async runTestLoop(): Promise<void> {
        for (let i = 0; i < this._proxiesList.length; i++) {
            if (this._cancelRequested) {
                console.log("[CycleTester] Cancellation detected, stopping after current proxy completes");
                break;
            }
            ...
            do {
                ...
                console.log(`%c[CycleTester] ========== НАЧАЛО ПЕРЕКЛЮЧЕНИЯ ПРОКСИ ==========`, 'color: #ffaa00; font-weight: bold');
                console.log(`[CycleTester] Текущее время: ${new Date().toLocaleTimeString()}`);
                ...
                const waitForProfile = async (targetProfileId: string, maxAttempts: number = 15): Promise<boolean> => {
                    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                        ...
                        if (currentProfile === targetProfileId) {
                            console.log(`%c[CycleTester] ✓ Профиль подтверждён: ${currentProfile} (попытка ${attempt})`, 'color: #ffffaa');
                            return true;
                        }
                        console.log(`[CycleTester] Ожидание смены профиля: сейчас ${currentProfile}, ожидаем ${targetProfileId} (попытка ${attempt}/${maxAttempts})`);
                    }
                    console.log(`%c[CycleTester] ✗ Не удалось сменить профиль на ${targetProfileId}`, 'color: #ff0000');
                    return false;
                };
                ...
                const waitForProxy = async (targetProxyId: string, maxAttempts: number = 15): Promise<boolean> => {
                    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                        ...
                        if (currentProxyId === targetProxyId) {
                            console.log(`%c[CycleTester] ✓ Прокси подтверждён: ${currentProxyId} (попытка ${attempt})`, 'color: #00ff00');
                            return true;
                        }
                        console.log(`[CycleTester] Ожидание смены прокси: сейчас ${currentProxyId}, ожидаем ${targetProxyId} (попытка ${attempt}/${maxAttempts})`);
                    }
                    console.log(`%c[CycleTester] ✗ Не удалось сменить прокси на ${targetProxyId}`, 'color: #ff0000');
                    return false;
                };

                const beforeProfile = Settings.current?.activeProfileId;
                console.log(`%c[CycleTester] Текущий активный профиль ДО переключения: ${beforeProfile || 'unknown'}`, 'color: #888888');
                ...
                if (profile) {
                    ...
                    Settings.updateActiveSettings();
                    console.log(`[CycleTester] Профиль изменён на ${targetProfileId} через Settings`);
                } else {
                    console.error(`[CycleTester] Профиль ${targetProfileId} не найден`);
                }
                api.runtime.sendMessage({ command: CommandMessages.PopupChangeActiveProfile, profileId: targetProfileId });

                const profileOk = await waitForProfile(targetProfileId, 15);
                if (!profileOk) {
                    console.error(`[CycleTester] Критическая ошибка: не удалось переключить профиль`);
                }

                console.log(`%c[CycleTester] ШАГ 3: Установка активного прокси: ${proxy.name} (${proxy.protocol}://${proxy.host}:${proxy.port})`, 'color: #ffaa00');
                ...
                if (targetProxy) {
                    ...
                    SettingsOperation.saveAllSync(false);
                    Settings.updateActiveSettings();
                    console.log(`[CycleTester] Прокси ${proxy.id} установлен как default`);
                    if (environment.name !== "Firefox") {
                        try {
                            const proxyAPI = api.proxy;
                            if (proxyAPI) {
                                ...
                                console.log(`[CycleTester] Proxy set via API for ${proxy.name} (${environment.name})`);
                            } else {
                                console.warn(`[CycleTester] api.proxy not available, skipping API proxy set`);
                            }
                        } catch (e) {
                            console.warn(`[CycleTester] Failed to set proxy via API:`, e);
                        }
                    }
                } else {
                    console.error(`[CycleTester] Прокси ${proxy.id} не найден`);
                }
                api.runtime.sendMessage({ command: CommandMessages.PopupChangeActiveProxyServer, id: proxy.id });

                const proxyOk = await waitForProxy(proxy.id, 15);
                if (!proxyOk) {
                    console.error(`[CycleTester] Критическая ошибка: не удалось установить прокси`);
                }
                ...
                console.log(`%c[CycleTester] ШАГ 4: Ожидание применения прокси (${delay}мс для ${protocolUpper})...`, 'color: #ffaa00');
                await new Promise(resolve => setTimeout(resolve, delay));
                console.log(`%c[CycleTester] ========== ПЕРЕКЛЮЧЕНИЕ ЗАВЕРШЕНО ==========`, 'color: #ffffaa; font-weight: bold');

                testResult = await this.testCurrentProxy(proxy);
                if (this._directIpDetected && this._retryForDirectIp) {
                    console.log(`%c[CycleTester] 🔁 Прямой IP, повторная проверка прокси ${proxy.name}...`, 'color: #ff8800');
                    this._retryForDirectIp = false;
                    needRetry = true;
            ...
            if (!testResult) {
                console.error(`[CycleTester] No test result for proxy ${proxy.name}, skipping`);
                continue;
            }
            if (testResult.status === "cancelled") {
                console.log(`[CycleTester] Skipping cancelled proxy ${proxy.name}`);
                break;
            }
           ...
           // console.log("[DEBUG] CycleTester: about to save autoStatus", JSON.stringify(Settings.current.autoStatus));
        ...
        await new Promise<void>((resolve) => {
            api.runtime.sendMessage({ 
                ...
                console.log("[CycleTester] Switched to Direct profile after test");
                resolve();
            });
        });

        if (wasCancelled) {
            console.log("[CycleTester] Test cancelled");
    ...
    async testCurrentProxy(proxy: ProxyListItem): Promise<TestResult> {
        ...
        this._indirectTimeoutFired = false;
        this._extendedTimerStarted = false;
        if (this._extendedTimer) {
            clearTimeout(this._extendedTimer);
            this._extendedTimer = null;
        }
        ...
        if (this._cancelRequested) {
            console.log(`[CycleTester] Cancellation requested before test start for ${proxy.name}, returning cancelled`);
        ...
        console.log(`[CycleTester] Создание основной вкладки для прокси ${proxy.name}, время=${Date.now()}`);
        const mainTabId = await createTestTab(this._testSite);
        this._currentMainTabId = mainTabId;
        console.log(`[CycleTester] Основная вкладка открыта id=${this._currentMainTabId}, время=${Date.now()}`);

        setTimeout(async () => {
            console.log(`%c[CycleTester] Запуск проверки IP через прокси...`, 'color: #ff8800');
            ...
            if (ip) {
                if (this._directIp && ip === this._directIp) {
                    ...
                    console.log(`%c[CycleTester] ❌ IP-вкладка вернула ПРЯМОЙ IP: ${ip}`, 'color: #ff0000; font-weight: bold; font-size: 1.2em');
                } else {
                    this._hasIndirectSuccess = true;
                    console.log(`[CycleTester] IP через прокси: ${ip}`);
                    if (this._currentMainTabId) {
                        try {
                            await api.tabs.update(this._currentMainTabId, { active: true });
                            console.log(`[CycleTester] Основная вкладка ${this._currentMainTabId} активирована`);
                        } catch (e) {}
                    }
                }
            } else {
                console.log(`[CycleTester] IP-вкладка не вернула IP`);
            }
        ...
        this._mainTimer = setTimeout(async () => {
            if (!this._cancelRequested && !this._exactSuccessTriggered) {
                console.log(`%c[CycleTester] Первый таймаут (20 сек)`, 'color: #ffaa00');
                this._mainTabFailed = true;
                await this.decideFinalResult();
            }
        }, 20000);

        while (!this._exactSuccessTriggered && 
               !(this._mainTabFailed && this._ipCheckCompleted) &&
               !this._indirectTimeoutFired) {
            await new Promise(resolve => setTimeout(resolve, 100));
        }

        const closeStart = Date.now();
        await this.closeTestTabs();
        console.log(`[CycleTester] Закрытие вкладок заняло ${Date.now() - closeStart} мс, время после закрытия=${Date.now()}`);

        const latency = Date.now() - startTime;
        if (this._exactSuccessTriggered) {
            console.log(`%c[CycleTester] ✅ РЕЗУЛЬТАТ: ${proxy.name} = УСПЕХ (${latency}мс)`, 'color: #00ff00; font-weight: bold; font-size: 1.4em');
            ...
            console.log(`%c[CycleTester] ☑️ РЕЗУЛЬТАТ: ${proxy.name} = КОСВЕННЫЙ УСПЕХ (${latency}мс)`, 'color: #0088ff; font-weight: bold; font-size: 1.4em');
            return { proxyId: proxy.id, proxyName: proxy.name, status: "indirect", latencyMs: latency };
        } else {
            console.log(`%c[CycleTester] ❌ РЕЗУЛЬТАТ: ${proxy.name} = НЕ РАБОТАЕТ (${latency}мс)`, 'color: #ff3333; font-weight: bold; font-size: 1.4em');
    ...
    async fetchProxyIpViaTab(proxy: ProxyListItem): Promise<string | null> {
        ...
        if (!workingServices.length) {
            console.log(`%c[CycleTester] ❌ Нет доступных IP-сервисов`, 'color: #ff0000');
            return null;
        }
        const servicesToTry = workingServices.slice(0, 3);
        console.log(`[CycleTester] 🔍 Параллельная проверка IP через ${servicesToTry.length} сервисов для прокси ${proxy.name}`);
        ...
        const testService = async (serviceUrl: string): Promise<string | null> => {
            ...
            return new Promise<string | null>((resolve) => {
                ...
                createTestTab(freshUrl).then(async (tabId) => {
                    ...
                    console.log(`[CycleTester] IP-вкладка создана, ID=${ipTabId}, всего IP-вкладок=${this._currentIpTabIds.length}`);
                    if (VISUAL_MODE) {
        ...
        if (result) {
            console.log(`%c[CycleTester] ✅ Получен IP через прокси: ${result}`, 'color: #00ff00');
            return result;
        }
        console.log(`%c[CycleTester] ❌ Не удалось получить IP ни от одного сервиса`, 'color: #ff0000');
        return null;
    },
        ...
        if (webReq && webReq.onResponseStarted) {
            webListenerRef = function(details: any) {
                if (details.tabId === that._currentMainTabId && details.statusCode === 200 && !that._responseReceived200) {
                    console.log(`%c[CycleTester] ★ Получен ответ 200`, 'color: #00ff00');
        ...
        onTabUpdatedRef = async function(tabId: number, changeInfo: any) {
            ...
            if (changeInfo.favIconUrl && !that._faviconLoaded) {
                ...
                if (!isErrorFav && !isDataUrl && !isBrowserInternal) {
                    ...
                    if (isSameDomain) {
                        console.log(`%c[CycleTester] ★ Favicon загружен (HTTP) from ${favHost}`, 'color: #00ff00');
                        that._faviconLoaded = true;
                        that.checkExactSuccess();
                    } else {
                        console.log(`[CycleTester] ⚠️ Favicon from different domain ${favHost} (ignored, expected ${testHost})`);
                    }
                } else if (isDataUrl) {
                    console.log(`[CycleTester] ⚠️ Favicon data: URL (ignored, not a real favicon)`);
                } else if (isBrowserInternal) {
                    console.log(`[CycleTester] ⚠️ Favicon browser internal URI: ${favUrl.substring(0, 50)} (ignored)`);
                }
            }
            ...
            if (changeInfo.title) {
                ...
                if (isCertError && !that._responseReceived200) {
                    console.log(`%c[CycleTester] ★ Ошибка сертификата (прокси работает)`, 'color: #00ff00');
                    that._responseReceived200 = true;
                    that.checkExactSuccess();
                }

                if (isError && !that._responseReceived200 && !that._exactSuccessTriggered) {
                    console.log(`[CycleTester] ❌ Ошибка в заголовке`);
        ...
        if (api.webNavigation && api.webNavigation.onErrorOccurred) {
            onErrorRef = function(details: any) {
                if (details.tabId === that._currentMainTabId && details.frameId === 0 && !that._exactSuccessTriggered) {
                    console.log(`%c[CycleTester] ⚠️ Ошибка навигации`, 'color: #ffaa00');
                    that._mainTabFailed = true;
                    that.decideFinalResult();
                }
            };
    ...
    checkExactSuccess(): void {
        ...
        if (this._responseReceived200 || this._faviconLoaded) {
            console.log(`%c[CycleTester] ★ Точный успех!`, 'color: #00ff00; font-weight: bold');
    ...
    async decideFinalResult(): Promise<void> {
        ...
        if (this._ipCheckPromise && !this._ipCheckCompleted && !this._cancelRequested) {
            console.log(`[CycleTester] Ожидание завершения IP-проверки перед принятием решения...`);
        ...
        if (this._hasIndirectSuccess && !this._directIpDetected) {
            if (this._mainTabFailed) {
                console.log(`[CycleTester] Косвенный успех, основная вкладка уже провалилась, завершаем`);
                this._indirectTimeoutFired = true;
                this.cleanupTimers();
                await this.closeTestTabs();
                return;
            }
            if (!this._extendedTimerStarted && !this._indirectTimeoutFired) {
                this._extendedTimerStarted = true;
                console.log(`[CycleTester] Косвенный успех, запуск расширенного таймера (20 сек) для ожидания загрузки основной вкладки`);
                this._extendedTimer = setTimeout(async () => {
                    if (!this._exactSuccessTriggered && this._hasIndirectSuccess && !this._indirectTimeoutFired) {
                        console.log(`[CycleTester] Расширенный таймаут (20 сек) истёк, завершаем с косвенным успехом`);
                        this._indirectTimeoutFired = true;
                        this._mainTabFailed = true;
                        this._ipCheckCompleted = true;
                        this.cleanupTimers();
    ...
    cleanupTimers(): void {
        ...
        if (this._extendedTimer) { clearTimeout(this._extendedTimer); this._extendedTimer = null; }
        this._mainTabFailed = true;
    },
    ...
    async closeTestTabs(): Promise<void> {
        ...
        console.log(`[CycleTester] Закрытие тестовых вкладок: уникальных ID=${tabIdSet.size}, время=${Date.now()}`);
        for (const tabId of tabIdSet) {
            try {
                const tab = await api.tabs.get(tabId);
                const url = tab.url;
                await api.tabs.remove(tabId);
                console.log(`[CycleTester] Tab ${tabId} closed successfully, time=${Date.now()}`);
                await removeFromHistory(url);
            } catch (e: any) {
                console.log(`[CycleTester] Tab ${tabId} already gone, skipping`);
            }
        }
        ...
        this._currentIpTabId = null;
        this._currentIpTabIds = [];
        console.log(`[CycleTester] Все вкладки закрыты, время=${Date.now()}`);
    },

    async cancelTest(): Promise<void> {
        if (!this._isRunning) return;
        console.log("[CycleTester] Cancel requested - current proxy will complete, next ones will not start");
    ...
    async getDirectIpWithCancellation(): Promise<string | null> {
        ...
        while (!isResolved) {
            if (this._cancelRequested) {
                console.log("[CycleTester] Cancelled while getting direct IP");
                return null;
            }
    ...
    reset(): void {
        ...
        if (this._extendedTimer) clearTimeout(this._extendedTimer);
        this._extendedTimer = null;
        this._extendedTimerStarted = false;
        this._indirectTimeoutFired = false;
        this._ipCheckResolve = null;
        ...
        if (this._cancelMessageListener) {
            ...
            console.log(`[CycleTester] Cancellation message listener removed`);
        }
        if (this._windowRemovedListener) {
            api.windows.onRemoved.removeListener(this._windowRemovedListener);
            this._windowRemovedListener = null;
            console.log(`[CycleTester] Window close listener removed`);
        }
        ...
        onTabUpdatedRef = null;
        onErrorRef = null;
        this.closeTestTabs().catch(err => console.warn("[CycleTester] Error closing leftover tabs during reset:", err));
    }
};
console.log("[ProxyMust] ProxyCycleTester.ts loaded successfully (BACKGROUND MODE)");